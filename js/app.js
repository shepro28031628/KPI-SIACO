// ---------- Global State ----------
      ...
      function getLineDatasets(rows, years, field, dateField = null) {
        const datasets = [];
        years.forEach((yr, idx) => {
          ...
          for (let m = 0; m < 12; m++) {
            if (counts[m] > 0) data[m] = parseFloat((sums[m] / counts[m]).toFixed(2));
          }
      ...
      const ChartManager = {
        ...
        renderModuloKPI(config) {
          ...
          const valoresTiempo = rows.map(r => r[config.campoTiempo]).filter(v => isNum(v) && numVal(v) >= 0);
          const tiempoAvg = valoresTiempo.length ? valoresTiempo.reduce((a, b) => a + numVal(b), 0) / valoresTiempo.length : 0;
          ...
          if (elDT) elDT.textContent = fmtInt(rows.length);

          const years = getYearsForRows(rows);
          this.renderLineChart(config.chartLinea, getLineDatasets(rows, years, config.campoTiempo, config.campoFecha));
